...
// ─── FX Rate ───────────────────────────────────────────────────────────
const NZD_AMOUNTS = [12.50, 2.50, 1.00];
...
async function fetchFX() {
  try {
    ...
    document.getElementById('fx-rate').innerHTML =
      `1 NZD = <span>$${rate.toFixed(4)} USD</span>`;

    NZD_AMOUNTS.forEach((nzd, i) => {
      document.getElementById(FX_IDS[i]).innerHTML = fmtUSD(nzd * rate);
    });
