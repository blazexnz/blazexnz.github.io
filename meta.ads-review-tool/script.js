...
const PHASES = [
  {
    ...
    actions: [
      ...
      "Promote emerging winners to static ad campaign",
      "Iterate 3-5 designs on emerging winners, as well as new design concepts",
  ...
  },
  {
    ...
    num: "02",
    name: "Scaling",
    description: "Promote profitable concepts to build up engagement, while maintaining stable ROAS, CPA, and creative performance.",
    objectives: [
      "Entry criteria: <$1 CPC on overall catalogue ad.",
      "Move high performing concepts from catalogue ad to static ad (purchases or low CPC in catalogue ad)",
      "Run static ad tests Thu–Sun at $21/day",
      "Maintain ROAS while gradually increasing daily ad budget",
      "Keep the budget low until there's profit",
    ],
    actions: [
      "Promote emerging winners from catalogue ad/purchases (exact product id - t-shirt colour etc.)",
      "$12",
      "Launch 3-5 iterations of winning concepts weekly",
      "Winners: any purchases and/or <$1.70 CPC -> leave them running",
      "Non-winners: no purchases and/or >$1.70 CPC -> turn off static ad",
      "Refresh creatives when CTR declines",
      "Gradually increase budgets if ROAS 2.0x",
      "Continue testing phase, while scaling profitable concepts",
    ],
    metrics: [
      { key: "roas", name: "ROAS", target: "≥ 2.0×", suffix: "×", operator: "gte", threshold: 2.0, higherIsBetter: true },
      { key: "cpa", name: "Cost Per Purchase (CPA)", target: "< $51", prefix: "$", operator: "lt", threshold: 51.00, higherIsBetter: false, decimal: true },
      { key: "cpc", name: "Cost Per Click (CPC)", target: "< $1.70", prefix: "$", operator: "lt", threshold: 1.70, higherIsBetter: false, decimal: true },
      { key: "ctr", name: "CTR", target: "≥ 2.5%", suffix: "%", operator: "gte", threshold: 2.5, higherIsBetter: true },
      { key: "frequency", name: "Frequency", target: "< 3.5", operator: "lt", threshold: 3.5, higherIsBetter: false },
      { key: "link_clicks", name: "Link Clicks", target: "≥ 10", operator: "gte", threshold: 10, higherIsBetter: true },
    ],
  },
  {
    id: "optimising",
